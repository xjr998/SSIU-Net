...
def main(opt):
    ...
    # three components are trained separately
    model1 = SSIU_Net()
    model2 = SSIU_Net()
    model3 = SSIU_Net()
    ...
    with open(opt.data_path, "r") as f:
        for line in f.readlines():
            ...
            if os.path.splitext(line)[1] == ".ply":
                ...
                log_string(LOG_FOUT, str((len(unique_idx) / pointNum)))      # Prints out the utilization rate of points 
                ...
                for j in range(numPatch):
                    ...
                    for t, m in enumerate(curIdx):
                        ...
                        # Points with duplicates are concated first, and then the mean is calculated at the end
                        else:
                ...
                # For the repeatedly selected points, the mean is calculated as the final predicted value
                ...
                mask = output_color_y == 0          # For points that are not selected, the value is directly assigned to the reconstructed point
