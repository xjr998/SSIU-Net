...
if __name__ == '__main__':
    # modify the bit rate of training data here directly
    bits = 'R01_'
    ...
    parser.add_argument('--checkpoint_y', type=str, default='./pths/2021-12-18/r02_manual_people_y_UNet/model_40.pth')
    parser.add_argument('--checkpoint_u', type=str, default='./pths/2021-12-18/r02_manual_people_y_UNet/model_40.pth')
    parser.add_argument('--checkpoint_v', type=str, default='./pths/2021-12-18/r02_manual_people_y_UNet/model_40.pth')
    ...
    # Modify the network framework directly here
    model1 = SSIU_Net().to(device)
    model2 = SSIU_Net().to(device)
    model3 = SSIU_Net().to(device)
    # if opt.arch == 'ARCNN':
    #     model = ARCNN()
    # elif opt.arch == 'VRCNN':
    #     model = VRCNN()
    ...
    if opt.resume == True:
        checkpoint_y = torch.load(opt.checkpoint_y)
        checkpoint_u = torch.load(opt.checkpoint_u)
        checkpoint_v = torch.load(opt.checkpoint_v)
        start_epoch = checkpoint_y['epoch'] + 1
        model1.load_state_dict(checkpoint_y['model_state_dict'])
        optimizer_Y.load_state_dict(checkpoint_y['optimizer_state_dict'])
        model2.load_state_dict(checkpoint_u['model_state_dict'])
        optimizer_U.load_state_dict(checkpoint_u['optimizer_state_dict'])
        model3.load_state_dict(checkpoint_v['model_state_dict'])
        optimizer_V.load_state_dict(checkpoint_v['optimizer_state_dict'])
    ...
    for epoch in range(opt.num_epochs):
        ...
        with tqdm(total=(len(train_dataset) - len(train_dataset) % opt.batch_size)) as _tqdm:
            ...
            for data in train_dataloader:
                ...
                ## train process
                preds_Y = model1(torch.unsqueeze(inputs[:, 0, :, :], -3))  # [B,1,32,32]
                preds_U = model2(torch.unsqueeze(inputs[:, 1, :, :], -3))  # [B,1,32,32]
                preds_V = model3(torch.unsqueeze(inputs[:, 2, :, :], -3))  # [B,1,32,32]
