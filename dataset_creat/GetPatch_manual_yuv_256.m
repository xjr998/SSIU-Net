...
for i=1:sequence_number
    for jj=1:length(rate)
        ...
        h5create([h5_train,ori_onlyName,rate{jj},'.h5'],'/data_hui',[16,16,3,num_Sample]);
        h5create([h5_train,ori_onlyName,rate{jj},'.h5'],'/label_hui',[16,16,3,num_Sample]);
        box_label_train_hui=zeros(16,16,3,num_Sample);
        box_data_train_hui=zeros(16,16,3,num_Sample);
        ...
        [idxnn_ori,dis]=knnsearch(kdtreeObj_ori,centroid_loc,'k',256);   % 索引按照距离centroid的由小到大的顺序排列,[num_sample,1024]
        ...
        for j=1:num_Sample
            ...
            curPatchCol_ori_hui=rectangular(curPatchCol_ori,16);
            ...
            curPatchCol_rec_hui=rectangular(curPatchCol_rec,16);
